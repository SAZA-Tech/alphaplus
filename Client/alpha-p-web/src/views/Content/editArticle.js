...
import {
  ...
  EDIT_ARTICLE,
  DELETE_ARTICLE,
} from "../../graphql/Content/articleGql";
...
function EditArticle(props) {
  ...
  const [saveDraft, { loading: saveLoading }] = useMutation(EDIT_ARTICLE, {
    ...
    onCompleted(data) {
      ...
      setSuccessMessage("article is edited");
  ...
  const [deletearticle, { loading: deleteLoading }] = useMutation(
    DELETE_ARTICLE,
    {
      onError(err) {
        console.log(`Error on ${err}`);
      ...
      onCompleted(data) {
        setSuccessMessage("article is deleted");
  ...
  return fetchLoading ? (
    ...
    >
      ...
      <div style={{ textAlign: "center", margin: "2rem" }}>
        {saveLoading | deleteLoading ? <CircularProgress /> : buttons()}
      </div>
      {success && (
        <div>
